# Atto 4 — trascrizione giocata «B» (autonomia → avviso centrale → focus lago; Log Lady, Sarah fra diner e Roadhouse)
...
- `m8.a.leland_waiting.p02` [m8_leland_waiting] **COOPER** — Non c'è fretta. Aspetterò che finiscano.
...
- _obiettivo (dopo l'interazione)_: Passa dal diner, questo pomeriggio.
- **scelta presa**: `promise_autonomia`
- `m8.a.diner.feedback.autonomia` [m8_diner] **MADDY** — (annuisce, piano) È la prima persona in questa città che me lo dice senza un "ma".
- _stato_: {"values":{"promise_stance":"autonomia"}}
...
- `m8.b0.leland_taxi.autonomia.p01` [m8_leland_taxi] **COOPER** — Maddy ha scelto Missoula. Non aggiungo un «ma»: verifico gli orari e le lascio la partenza.
...
- _tasto A_ @diner 4,6 up
- _dialogo classico_ `loglady_a4` su diner: LOG LADY: Il mio ceppo ha ripreso
a parlare, agente. / LOG LADY: Il mio ceppo dice:
stanotte, al roadhouse. / LOG LADY: Le civette sono gia li. / LOG LADY: Vada, agente. Prima
che la musica cominci. / COOPER: Margaret, questo basta:
luogo e ora. Ci saro.
- _obiettivo (dopo l'interazione)_: Il paese si ritrova al Roadhouse, stasera.
- _viaggio_ → palmer 7,10,up (viaggio: casa Palmer (fra il diner e il Roadhouse))
...
- `m8.b.truman.p06` [m8_roadhouse_truman] **COOPER** — (a bassa voce) Ancora, allora. Non so ancora che forma prenderà.
...
- `m8.b.giant.p02` [m8_giant_stage] **COOPER** — (sottovoce) Nessuno guarda dove guardo io.
- _commit_ `m8_giant_stage`
- _stato_: {"nodes_done":["m8_giant_stage"]}
- ⚠️ **API** `roadhouse/gigante` — NPC "gigante" a 8,1 presente ma nessuna casella adiacente raggiungibile: {"ok":false,"error":"unreachable","target":[8,1],"candidates":[[8,2,"up"]],"diag":{"from":[3,8],"to":[8,2],"reason":"no_path","attempt":4}} · NPC: ["truman@4,8","bobby@3,2","donna@5,2","james@9,2","shelly@11,2","norma@3,6","loglady@11,6","gigante@8,1"]
- `m8.b.giant.repeat` [m8_giant_stage] **COOPER** — È ancora sul palco. Nessun altro lo nota, mai.
...
- _obiettivo (dopo l'interazione)_: Il telefono del Roadhouse.
- **scelta presa**: `warning_centrale`
- `m8.b.roadhouse.feedback.centrale` [m8_roadhouse_phone] **LUCY** — Glielo passo— no, è di pattuglia. Mando Andy a casa Palmer, agente. Ci mette dieci minuti.
- _stato_: {"values":{"warning_target":"centrale","maddy_action_after_warning":"none","sarah_support_state":"vice"}}
...
> valori dopo: {"promise_stance":"autonomia","presagio_status":"active","warning_target":"centrale","maddy_action_after_warning":"none","sarah_support_state":"vice"}
...
- **scelta presa**: `focus_lago`
- _obiettivo (dopo l'avanzamento)_: Torna all’incrocio: casa Palmer, lago o diner.
- _tasto A_ @town 16,28 left
- `m8.c.route_lake.p01` [m8_route_lake] _(scena)_ — (L'acqua è ferma. Sulla riva, una forma che l'acqua non reclama. Cooper arriva per primo.)
- `m8.c.route_lake.p02` [m8_route_lake] **COOPER** — Fermo al perimetro. Prima proteggo la scena; poi troverò le parole per Maddy.
- _commit_ `m8_route_lake`
- _stato_: {"nodes_done":["m8_route_lake"],"values":{"body_found_by":"cooper"}}
- _obiettivo (dopo l'interazione)_: Torna all’incrocio: casa Palmer, lago o diner.
- _tasto A_ @town 16,28 left
- `m8.d.discovery.cooper.p01` [m8_discovery] _(scena)_ — (Sulla riva: Maddy. Cooper si ferma prima del perimetro.)
- `m8.d.discovery.cooper.p02` [m8_discovery] **COOPER** — (fermo) Signorina Ferguson. Maddy. Con due D.
- `m8.d.discovery.cooper.p03` [m8_discovery] _(scena)_ — (Si china senza toccare. Sotto l'unghia dell'anulare: un segno.)
- `m8.d.discovery.cooper.p04` [m8_discovery] _(scena)_ — (Arrivano le torce di Hawk e del vice. Cooper non si è mosso di un passo.)
...
- `m8.d.echo.autonomia` [m8_promise_echo] **COOPER** — Diane. Disse che Missoula l'aspettava senza un "ma".
...
- _stato_: {"flags":["maddy_trovata"],"evidence":["E9A_LETTERA_O","E9B_STESSO_METODO"],"nodes_done":["m8_discovery","m8_promise_echo"],"values":{"presagio_status":"verified","letter_o_observation_source":"cooper_primary","letter_o_chain":"standard"}}
...
- **scelta presa**: `diary_c`
- `m8.e.cmp_diary.feedback.c` [m8_cmp_diary] _(scena)_ — Il diario nomina "lui". Non dice chi, né dove abita.
- **scelte** [m8_cmp_diary] @town 16,28,left: diary_a · diary_b
...
- `m8.f.station.sarah_vice` [m8_station] **TRUMAN** — Andy era già con Sarah quando è arrivata la chiamata.
- `m8.f.station.p_lago` [m8_station] **TRUMAN** — La chiamata sulla riva è arrivata qui mentre tu eri già al lago. Un civile, senza nome; Hawk è partito subito.
