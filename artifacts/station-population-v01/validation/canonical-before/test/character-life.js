...
class FakeImage {
  ...
  set src(value) {
    this._src = value;
    const life = value.includes('character-life-v01.png');
    this.naturalWidth = life ? 96 : 360;
    this.naturalHeight = life ? 24 : 360;
    this.complete = true;
...
assert(rendered.draw[0].src.includes('character-life-v01.png'));
...
// Production station data adds one still NPC and preserves all prior geometry and World assets.
assert.deepEqual(G.SheriffsStationScene.map.npcs, [{
  id: ACTOR_ID, x: 10, y: 4, sprite: 'truman', name: 'Truman', dir: 'down', wander: false
}]);
...
assert.deepEqual(G.SheriffsStationScene.map.rows, Array.from(oldSceneContext.GAME.SheriffsStationScene.map.rows), 'station collision geometry is unchanged');
...
assert.deepEqual(G.SheriffsStationScene.layout.targets, JSON.parse(JSON.stringify(oldSceneContext.GAME.SheriffsStationScene.layout.targets)));
...
for (const file of [
  'js/sheriffs-station-art.js', 'js/sheriffs-station-production.js', 'js/world-engine.js',
  'js/world-catalog.js', 'js/maps.js', 'js/ambient-life.js', 'assets/sprites/cast-walkcycles-hg-24.png'
]) {
...
assert.deepEqual(Object.keys(G.Retro2D.characterLifeFrames.frames), Object.keys(manifest.frames), 'renderer frame vocabulary matches manifest');
