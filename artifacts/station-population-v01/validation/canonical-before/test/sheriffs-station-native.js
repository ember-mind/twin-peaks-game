...
const MAP_ID = 'sheriffs_station_main_interior';
...
assert.strictEqual(G.Maps[MAP_ID], Scene.map);
assert.equal(Scene.map.width, 16);
assert.equal(Scene.map.height, 12);
assert.equal(Scene.map.width * 16, 256, 'map spans the full native viewport width');
assert.equal(Scene.map.rows.every((row) => row.length === Scene.map.width), true);
assert.equal(Scene.map.indoor, true);
assert.deepEqual(Scene.map.doors, {}, 'rear door is closed scenery, not a transition');

const isSolid = (x, y) => G.Maps.isSolid(MAP_ID, x, y, { clues: [], flags: {} });
const freeTiles = [];
for (let y = 0; y < Scene.map.height; y++) {
  for (let x = 0; x < Scene.map.width; x++) if (!isSolid(x, y)) freeTiles.push([x, y]);
}
...
assert(engineDepthIntervals.some(([min, max]) => min === 80 && max === Infinity), 'NPC foreground interval completes the remaining room depth');
