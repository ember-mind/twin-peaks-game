# Architecture options — named-character presence (lead evaluation, 2026-09-11; amended same day)

**Amendment.** Option C is kept as the runtime shape only. Its original resolution semantics ("the first rule whose `when` holds wins", "top-priority offscreen rule", "overlaps resolve silently by order") are **rejected** by the lead: overlapping predicates for one character are a contradictory authored world and a build failure, resolution is order-independent (all windows evaluated, exactly one match or baseline, else error), and the authoring source is Cast Continuity windows with causal change records. Normative text: `docs/cast-continuity-contract-v0.1.md`.
...
## Option C — Character presence rules (declarative, per character, derived) — RECOMMENDED AS RUNTIME SHAPE (semantics amended, see top)

A central cast registry: for every named character an **ordered list of presence rules** `{when, sceneId, x, y, dir, facing?, dialogue?}` plus a terminal default (home or `offscreen`). Resolution (original text, superseded): ~~for a given story state, the first rule whose `when` holds wins~~ → amended: all windows naming the character are evaluated; exactly one must match, otherwise the baseline applies, otherwise error; the result is a total function `state → placement | OFFSCREEN | TERMINAL_REMOVED` per character. Map population = classic *background* NPCs (unchanged) + the cast placements resolved to that map. Missions contribute rules for their windows (same `when` grammar as entities today, same validator), the registry owns the order and the defaults.
...
- Risks (amended): rules that overlap are a **build failure** (V2), never resolved by order and never declarable; the crowd's `dialogue: null` bodies become placements *with* the classic dialogue reachable (a design decision per character: keep `dialogue: null` per rule if the scene wants silence).

## Verdict

Option C. It is the smallest change that makes the failure class *unrepresentable* rather than *detectable*, keeps presence derived (no save migration), reuses the existing `when` grammar and validator, and leaves World Engine and Character Life untouched. Option A is the natural instinct ("move him") and is rejected because it converts a property into a procedure and reintroduces phantoms by omission. Option B is rejected because the game has no phase lattice and a scene-scale appearance does not fit a phase — but the amended model borrows B's strongest idea: authoring and pinning whole-window cast snapshots (V5), without requiring a total phase order (windows only need per-character exclusion). Option 0 is the `gigante2` hack at scale.
