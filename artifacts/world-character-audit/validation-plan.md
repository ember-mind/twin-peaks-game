...
| RC7 gathering entry (B1, closed) | 7 room empties on a flag while the player is inside | before: `promise_stance ∧ ¬T_LELAND_TAXI`; after: `T_LELAND_TAXI ∧ ¬presagio_status` | before: norma/shelly/loglady/james → `diner`; after: → `roadhouse`. The gathering window must carry `entry_authored_by: m8.b0.leland_taxi.chiusura.p01` for the four, and that page must exist in `m8_leland_taxi` (map `diner`, mode `action`, before the notebook page). Seeded failures: (i) page id missing from M8 → FAIL; (ii) `entry_authored_by` absent → FAIL; (iii) the beat authored in a node on another map → FAIL | V5, V6b |
