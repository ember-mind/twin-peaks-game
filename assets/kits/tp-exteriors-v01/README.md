...
`engine/tools/grade-kit.py` (the shared grader) makes the day and night grades. `grade-fix.py` then regrades surfaces that only look warm, such as the cream siding and painted lines, so they read as unlit, and gives the backdrop an overcast day sky. The day grade of `light-spill` is left empty.
...
| `objects/` | Cut from the plates: `double-r` (door, open-door frame, windows), `sheriff-station` (same), `rr-sign`, `planter-l`/`-r`, `bin`, `bench`, `shrub-1..4`, `flagpole`, `wheel-stop`, `stall-line`, `asphalt-patch-1..4`, `light-spill`, `wall-lamp`. Drawn in the concepts' palette and manner: `pine-s/m/m2/l/l2/xl`, `street-lamp` + `lamp-pool`, `fence` + `fence-post`, `bush-1/2`, `road-dash`, `backdrop` (dusk sky and three rows of dark pines). |
| `kit.json` | Same fields as Living Town's, plus `light.doorway` (the Twin Peaks open-door colours) and `light.windowPane: false`. The street lamp is `alwaysLit`. |
...
- **Open door and windows.** The engine's kit hooks handle these. `light.doorway` lights the open leaf in the diner's interior browns. It is set once for the whole kit, so the station uses the same colours. `light.windowPane: false` keeps the painted interiors. The page lights a place while someone is inside it, and only once dusk falls.
- **Street lights.** They use kind `streetlight`, not the engine's `lamp`, because the engine's pool is a soft ramp of stacked translucent rects. The kit's crisp `lamp-pool` decal lies under each light instead (faint at dusk, full at night, none by day). The lens is an `alwaysLit` window, so the lights still relight the cast. They have no head halo.
