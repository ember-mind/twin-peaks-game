...
1. Author a **window**: `id`, `owner` (mission), `when` (mission `when` grammar over existing story state: `flag`, `evidence`, `node_done`, `value_set`, `value_is`, `proposition_path`, `not`, `all`; no `any`: split into value-exclusive windows), `cast` (only the characters that change; each a placement `{status: PLACED, map_id, x, y, dir, dialogue, actor_ids?}` / `{status: OFFSCREEN, label}` / `{status: TERMINAL_REMOVED, event}`; `actor_ids` only when a mission actor node addresses this body by another id, e.g. `hawk_bridge`), `cause` (one line: the story event that moves them), `exit` (the event that ends the window).
...
6. Run `node test/cast-continuity-validate.js`, then the real-build drivers (`test/act-3-playthrough.js`, `test/act-4-playthrough.js`) whose Cast Presence snapshots compare against the pins. Synthetic seeds in harnesses must be reachable states (copy a fixture seed): the resolver hard-errors on contradictory histories. V1–V8 and the regression corpus must be green. Never re-pin a fixture to make a runtime defect green.
