...
9. Environment brief pointer(s) (spatial relationships that carry meaning; targets; states).
...
PASS: every mandatory beat in §6 has a state writer with a reader (or INTENTIONAL-UNRESOLVED), a channel, and a character function; no class-D pressure unargued; §5 cites lines; §14 tags every task.
