 ...
 * crossing stands on the approach tile and presses the arrow key into the trigger, exactly as a player does.
 * Checks: the gate message when the flag/clue count is missing, the arrival map + spawn + facing, every leaf of
 * a two-tile door, and that one-way records have no way back.
 */
...
function setScene(mapId, tx, ty, dir) {
  ...
  pump(800); // clear the bump-message cooldown between attempts
}
function where() { return [E.state.mapId, E.state.player.tx, E.state.player.ty, E.state.player.dir]; }

// Walk from (mapId, stand) pressing dir into the trigger; expect arrival at [to, tx, ty, facing].
...
// Pressing into a gated trigger without its requirement shows the authored message and stays put.
...
// ---- gates first, with nothing unlocked
...
// ---- one-way migrated crossings and no way back
