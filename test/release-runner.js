...
for (const value of ['npm test', 'node test/../secret.js', 'node test/a.js; echo pass', '|', '']) {
  assert.throws(() => commandsFromWorkflow('  run: ' + value));
}
...
try {
  ...
  fs.writeFileSync(path.join(root,'.github/workflows/test.yml'), '  run: node test/pass.js\n  run: node test/fail.js\n  run: node test/hang.js\n');
  ...
  console.log('release-runner: parser, failure, timeout, evidence and no-overwrite contracts passed');
